...
const LandingPage = (props) => {
  ...
  const dateCellRender = (value) => {
    ...
    if (diaries.length > 0) {
      dateItem = diaries.find(
        (diary) =>
          diary.post_year === year &&
          diary.post_month === month &&
          diary.post_day === day
      );
    }
    return <>{dateItem && dateItem.content}</>;
  };
  ...
  const monthCellRender = (value) => {
    ...
    let year = parseInt(date[0]);
    if (diaries.length > 0) {
      monthItem = diaries.filter(
        (diary) => diary.post_year === year && diary.post_month === month
      );
    }
